...
public $data;
  ...
  function allRolUser(){
   ...
   function allUser(){
    // $sql= "SELECT g.id, g.user_name, g.active, r.name as name_rol FROM `user` g, `rol` r where g.id_rol=r.id AND `delete`='N' ORDER BY g.user_name ASC ";
                        ...
                        <table class="data-table table stripe hover nowrap">
                            ...
                            <tbody  >
                                ...
                                <tr>
                                    ...
                                    <td><?php if($img['active']== 1){echo 'Activo';} else {echo 'Inactivo';}; ?></td>
                                    <td>
                                        <div class="dropdown">
...
//          }
   ...
   function excelCliente($user_name){
    ...
    $sql = "SELECT COUNT(id), create_date, name_cliente FROM path WHERE name_cliente='".$user_name."' OR path LIKE '%".$user_name."%' GROUP BY create_date";
...
// exit();
    ...
    function arrAllPathAndDataOfImgCliente($name_cliente,$create_date){
        ...
        $sql = "SELECT p.path, p.hora, p.description, u.user_name as monitorista, p.hora_f FROM `path` p, `user` u WHERE p.name_cliente='".$name_cliente."' AND p.create_date='".$create_date."' AND p.id_monitor=u.id ORDER BY p.hora"; 
         ...
         // 
        $sql= "SELECT p.path, p.hora, p.description, p.hora_f FROM `path` p WHERE p.name_cliente='".$name_cliente."' AND p.create_date='".$create_date."'  ORDER BY p.hora";
